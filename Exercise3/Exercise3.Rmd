...
```{r ans12}
classData[classData$`公-10-06`=="有看公告",] -> subclassdata
```
...
```{r ans22}
ymd_hms("2018-10-24 00:00:00",tz="Asia/Taipei") -> deadline
```
...
```{r ans25}
hwData[(hwData$Timestamp<deadline & hwData$作業代碼=="hw1"),] -> subhwdata
table(subhwdata$early)
```
